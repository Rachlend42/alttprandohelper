        ...
        return update(dungeons, {
            ...
            darkness: { $merge: {
                ...
                can_complete: function(items, model) {
                    return this.can_enter(items, model) && items.has_bow() && items.hammer &&
                        this.big_key && this.keys ?
                            items.lantern ?
                                this.keys === 6 ? 'available' : 'possible' :
                                'dark' :
                        'unavailable';
                },
                can_progress: function(items, model) {
                    ...
                    var advance = function(count, condition, delta) {
                        return !condition ? count :
                            map_values(count, function(value, key) { return value + (delta[key] || 0); });
                    };

                    var count = { keys: this.keys, reachable: 0, dark: 0 };
                    count = advance(count, true, { reachable: 1 }); // left side
                    count = advance(count, items.has_bow(), { reachable: 2 }); // bow locked right side
                    // bridge and dropdown, with/without front door opened
                    count = advance(count, items.has_bow() && items.hammer, { reachable: 2 });
                    count = advance(count, !(items.has_bow() && items.hammer) && count.keys, { keys: -1, reachable: 2 });
                    // back of POD, since it yields most chests for one key
                    count = advance(count, count.keys, { keys: -1, reachable: 3, dark: 2 });
                    // dark maze
                    count = advance(count, count.keys, { keys: -1, reachable: this.big_key ? 3 : 2, dark: this.big_key ? 3 : 2 });
                    // helmasaur. we do not prioritize him when he is beatable. This way we show the max amount of items.
                    count = advance(count, count.keys && this.big_key && items.has_bow() && items.hammer, { keys: -1, reachable: 1, dark: 1 });
                    count = advance(count, count.keys, { keys: -1, reachable: 1 }); // spike room
                    count = advance(count, count.keys, { keys: -1, reachable: 1 }); // big key chest
                ...
                doors: {
                    ...
                    front: { caption: 'Front', second_map: true, can_reach: always },
                    big_key: { caption: 'Big Key', second_map: true, can_reach: always }
                },
        ...
        return update(chests, {
            ...
            $merge: {
                ...
                castle_maze: {
                    ...
                    is_available: function(items, model) {
                        return model.regions.castle_tower.keys && (items.sword >= 2 || items.cape && items.sword) ?
